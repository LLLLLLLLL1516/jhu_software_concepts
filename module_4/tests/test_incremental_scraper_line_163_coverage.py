    ...
    with patch('psycopg.connect', return_value=mock_conn):
        with patch('urllib3.PoolManager', return_value=mock_http):
            with patch('time.sleep'):  # Speed up execution
                ...
                scraper._parse_list_page = mock_parse
    ...
    with patch('psycopg.connect', return_value=mock_conn):
        with patch('urllib3.PoolManager', return_value=mock_http):
            with patch('time.sleep'):
                ...
                def mock_parse(self, html):
                    call_count[0] += 1
                    if call_count[0] == 1:
                        return [
                            {'program': f'Prog{i}', 'status': 'A', 'date_added': 'Sep 20, 2025'}
                            for i in range(50)
                        ]
                    return []
                
                incremental_scraper.IncrementalGradCafeScraper._parse_list_page = mock_parse
                ...
                try:
                    ...
                    with patch('builtins.open', mock_open()) as m_open:
                        ...
                        m_open.assert_called_with("new_applicant_data.json", 'w', encoding='utf-8')
                        
                finally:
                    # Restore original
                    incremental_scraper.IncrementalGradCafeScraper._parse_list_page = original_parse

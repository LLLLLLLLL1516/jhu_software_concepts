...
from flask_app import scraping_status, create_app


class InstantStateController:
...
def test_complete_workflow_fake_scraper_to_analysis(mock_psycopg_connect, mock_db_connection):
    """Test complete workflow: fake scraper → POST /pull-data → POST /update-analysis → GET /analysis"""
    ...
    with patch('flask_app.threading.Thread') as MockThread:
        ...
        with patch('flask_app.GradCafeQueryAnalyzer', MockGradCafeQueryAnalyzerForIntegration):
            ...
            assert integration_scraper.scraped == True
...
def test_uniqueness_preservation_across_multiple_pulls(mock_psycopg_connect, mock_db_connection):
    ...
    with patch('flask_app.threading.Thread') as MockThread:
        ...
        assert final_count >= count_after_first, "Should have at least as many records as after first pull"
        ...
        for percentage in percentages:
            assert re.match(r'^\d+\.\d{2}%$', percentage), f"Percentage '{percentage}' should maintain format after multiple pulls"


@pytest.mark.integration
def test_error_handling_in_complete_workflow(mock_psycopg_connect):
    ...
    class FailingScraper:
        def scrape(self):
            ...
            scraping_status['is_running'] = False
            raise Exception("Integration test scraper failure")
...
@pytest.mark.integration
def test_data_pipeline_integration_with_formatting(mock_psycopg_connect, mock_db_connection):
    ...
    class PreciseFormatMockAnalyzer:
        def __init__(self, db_config):
            self.db_config = db_config
            self.connected = False
            self.results = {
    ...
    with patch('flask_app.threading.Thread') as MockThread:
        ...
        with patch('flask_app.GradCafeQueryAnalyzer', PreciseFormatMockAnalyzer):
            ...
            assert len(wrong_format_percentages) == 0, f"Found incorrectly formatted percentages: {wrong_format_percentages}"
            
            # Verify all percentages have exactly 2 decimal places
            all_percentages = re.findall(r'\b\d+\.\d{2}%', html_content)
            assert len(all_percentages) >= 2, "Should find at least 2 correctly formatted percentages"

...
@pytest.fixture
def mock_http_pool(sample_html_response):
    """Mock HTTP pool manager with sample responses"""
    responses = {
        'survey/index.php': {
            'status': 200,
            'data': sample_html_response
        },
        'robots.txt': {
            'status': 200,
            'data': 'User-agent: *\nAllow: /'
...
class TestIncrementalGradCafeScraper:
    ...
    def test_init(self, mock_db_config, mock_http_pool):
    ...
    def test_init_default_config(self, mock_http_pool):
    ...
    @pytest.mark.db
    def test_get_latest_database_date_success(self, mock_db_config, mock_http_pool):
    ...
    @pytest.mark.db
    def test_get_latest_database_date_no_data(self, mock_db_config, mock_http_pool):
    ...
    @pytest.mark.db
    def test_get_latest_database_date_error(self, mock_db_config, mock_http_pool, capsys):
    ...
    def test_is_entry_newer_various_formats(self, mock_db_config, mock_http_pool):
    ...
    @pytest.mark.analysis
    def test_is_entry_newer_edge_cases(self, mock_db_config, mock_http_pool, capsys):
    ...
    @pytest.mark.analysis
    def test_is_entry_newer_exception(self, mock_db_config, mock_http_pool, capsys):
        ...
        with patch('urllib3.PoolManager', return_value=mock_http_pool):
            ...
            with patch('incremental_scraper.datetime') as mock_datetime:
                ...
                captured = capsys.readouterr()
                assert "Error parsing date" in captured.out
    
    @pytest.mark.integration
    def test_scrape_new_data_only_success(self, mock_db_config, sample_html_response):
    ...
    @pytest.mark.integration
    def test_scrape_new_data_only_no_existing_data(self, mock_db_config, mock_http_pool, capsys):
        ...
        with patch('urllib3.PoolManager', return_value=mock_http_pool):
            with patch('psycopg.connect', return_value=MockPsycopgConnection(None)):
                ...
                assert results == []
                captured = capsys.readouterr()
                assert "No existing data found. Consider running full scraper first." in captured.out
    
    @pytest.mark.integration
    def test_scrape_new_data_only_consecutive_old_pages(self, mock_db_config):
    ...
    @pytest.mark.integration
    def test_scrape_new_data_only_request_failure(self, mock_db_config):
    ...
    @pytest.mark.integration
    def test_scrape_new_data_only_empty_page_results(self, mock_db_config):
    ...
    @pytest.mark.integration
    def test_scrape_new_data_only_progress_update(self, mock_db_config, capsys):
        ...
        html_with_many_entries = """<html><body><table class="tw-min-w-full"><tbody>"""
        for i in range(50):  # Changed from 51 to 50
            html_with_many_entries += f"""
            <tr>
                <td>University {i}</td>
                <td><div class="tw-text-gray-900"><span>CS</span></div></td>
                <td>September 20, 2025</td>
            ...
            """
        html_with_many_entries += """</tbody></table></body></html>"""
    ...
    def test_save_new_data_success(self, mock_db_config, mock_http_pool):
    ...
    def test_save_new_data_error(self, mock_db_config, mock_http_pool, capsys):
        ...
        with patch('urllib3.PoolManager', return_value=mock_http_pool):
            ...
            # Mock json.dump to raise an exception to trigger the except block
            with patch('json.dump', side_effect=Exception("JSON encoding failed")):
...
class TestMainFunction:
    ...
    def test_main_with_new_results(self, capsys):
        ...
        mock_results = [
            ...
            {'program': 'Data Science', 'status': 'Interview', 'date_added': 'Sep 18, 2025'},
    ...
    def test_main_no_results(self, capsys):
    ...
    def test_main_with_entries_missing_fields(self, capsys):
...
with patch('incremental_scraper.IncrementalGradCafeScraper', return_value=mock_scraper):
    ...
    import runpy
    runpy.run_path(os.path.join("{test_dir}", "..", "src", "incremental_scraper.py"), run_name="__main__")
'''
...
@pytest.mark.integration
def test_main_saves_data_to_file(capsys):
    ...
    mock_results = [
        {'program': 'Test Program', 'status': 'Accepted', 'date_added': 'Sep 20, 2025'}
    ]
    ...
    with patch('incremental_scraper.IncrementalGradCafeScraper', return_value=mock_scraper):
        ...
        from incremental_scraper import main
        result = main()

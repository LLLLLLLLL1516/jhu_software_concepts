...
class TestFlaskAppFinalCoverage:
    ...
    def test_run_data_pipeline_with_injected_scraper_exception(self):
        """Test data pipeline when injected scraper raises exception - covers lines 84-93 error path"""
        ...
        with app.test_client() as client:
            ...
            with patch('threading.Thread') as MockThread:
                ...
                assert scraping_status['error'] == 'Scraper failed'
                assert 'Scraper failed' in scraping_status['progress']
                assert scraping_status['is_running'] == False
    ...
    def test_complete_data_pipeline_with_db_success(self):
        """Test complete pipeline including successful database operations - ensures lines 67-69 are covered"""
        with patch('flask_app.psycopg') as mock_psycopg:

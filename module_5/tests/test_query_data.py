...
# Import at module level to avoid import-outside-toplevel issues
# pylint: disable=import-error,wrong-import-position
from query_data import GradCafeQueryAnalyzer, main
# pylint: enable=import-error,wrong-import-position


class TestGradCafeQueryAnalyzer:  # pylint: disable=too-many-public-methods
    ...
    def test_question_3_average_metrics(self):
        ...
        with patch.object(
            analyzer, "execute_query", return_value=(3.5, 155, 160, 4.5)
        ) as mock_execute:
            ...
            with patch.object(analyzer, "question_2_international_percentage") as mock_q2:
                with patch.object(analyzer, "question_3_average_metrics") as mock_q3:
                    with patch.object(analyzer, "question_4_american_fall_2025_gpa") as mock_q4:
                        with patch.object(
                            analyzer, "question_5_fall_2025_acceptance_rate"
                        ) as mock_q5:
                            with patch.object(
                                analyzer, "question_6_fall_2025_accepted_gpa"
                            ) as mock_q6:
                                with patch.object(analyzer, "question_7_jhu_cs_masters") as mock_q7:
                                    with patch.object(
                                        analyzer,
                                        "question_8_georgetown_cs_phd_2025_acceptances",
                                    ) as mock_q8:
                                        with patch.object(
                                            analyzer,
                                            "question_9_penn_state_international_fall_2025",
                                        ) as mock_q9:
                                            with patch.object(
                                                analyzer,
                                                "question_10_penn_state_2025_acceptances",
                                            ) as mock_q10:
...
with patch('psycopg.connect'):
    ...
    with patch('query_data.GradCafeQueryAnalyzer', return_value=mock_analyzer):
        ...
        import runpy
        runpy.run_path(
            os.path.join("{test_dir}", "..", "src", "query_data.py"), run_name="__main__"
        )
"""

...
class MockGradCafeQueryAnalyzerWithPercentages:
    ...
    def __init__(self, db_config):
        ...
        self.results = {
            "acceptance_rate": {
                "description": "What percentage of applications resulted in acceptance?",  # pylint: disable=line-too-long
                "query": 'SELECT COUNT(*) FROM applicant_data WHERE status = "Accepted"',
                "result": "15.25",
            },
            "rejection_rate": {
                "description": "What percentage of applications were rejected?",  # pylint: disable=line-too-long
                "query": 'SELECT COUNT(*) FROM applicant_data WHERE status = "Rejected"',
                "result": "84.75",
            },
            "international_percentage": {
                "description": "Percentage of international students",  # pylint: disable=line-too-long
                "query": 'SELECT COUNT(*) FROM applicant_data WHERE us_or_international = "International"',  # pylint: disable=line-too-long
                "result": "42.33",
            },
            "cs_program_percent": {
                "description": "Percent of CS programs in database",  # pylint: disable=line-too-long
                "query": 'SELECT COUNT(*) FROM applicant_data WHERE program LIKE "%Computer Science%"',  # pylint: disable=line-too-long
                "result": "28.50",
            },
            "avg_gpa": {
                "description": "Average GPA of all applicants",  # pylint: disable=line-too-long
                "query": "SELECT AVG(gpa) FROM applicant_data",
                "result": 3.75,
            },
            "total_count": {
                "description": "Total number of applications",  # pylint: disable=line-too-long
...
def test_percentage_format_with_two_decimals(client, mock_psycopg_connect):  # pylint: disable=unused-argument
...
@pytest.mark.analysis
def test_percentage_format_in_html_structure(client, mock_psycopg_connect):  # pylint: disable=unused-argument
        ...
        for div in result_divs:
            ...
            if "%" in text:
                ...
                for perc in percentages:
                    ...
                    assert (
                        len(decimal_part) == 2
                    ), f"Percentage '{perc}' should have exactly 2 decimal places"

        assert percentage_found, "Should find at least one percentage value in result displays"


@pytest.mark.analysis
def test_non_percentage_values_formatting(client, mock_psycopg_connect):  # pylint: disable=unused-argument
...
@pytest.mark.analysis
def test_percentage_calculation_accuracy(client, mock_psycopg_connect):  # pylint: disable=unused-argument
    ...
    with patch("flask_app.GradCafeQueryAnalyzer", MockGradCafeQueryAnalyzerWithPercentages):
        ...
        assert (
            len(found_percentages) > 0
        ), f"Should find some expected percentages: {expected_percentages}"


@pytest.mark.analysis
def test_percentage_regex_precision(client, mock_psycopg_connect):  # pylint: disable=unused-argument
    ...
    with patch("flask_app.GradCafeQueryAnalyzer", MockGradCafeQueryAnalyzerWithPercentages):
        ...
        if matches:  # Only test if percentages are found
            for match in matches:
                ...
                assert re.fullmatch(
                    r"\d{1,3}\.\d{2}%", match
                ), f"Percentage '{match}' doesn't match exact format"

                # Verify the decimal part has exactly 2 digits
                decimal_part = match.split(".")[1].replace("%", "")
                assert (
                    len(decimal_part) == 2
                ), f"Decimal part of '{match}' should have exactly 2 digits"


@pytest.mark.analysis
def test_template_percentage_rendering_logic(client, mock_psycopg_connect):  # pylint: disable=unused-argument
...
@pytest.mark.analysis
def test_mixed_data_types_formatting(client, mock_psycopg_connect):  # pylint: disable=unused-argument
...
@pytest.mark.analysis
def test_html_escaping_in_percentages(client, mock_psycopg_connect):  # pylint: disable=unused-argument
    ...
    with patch("flask_app.GradCafeQueryAnalyzer", MockGradCafeQueryAnalyzerWithPercentages):
        ...
        assert (
            "&amp;" not in html_content or "%" in html_content
        ), "Percentage symbols should not be double-escaped"
